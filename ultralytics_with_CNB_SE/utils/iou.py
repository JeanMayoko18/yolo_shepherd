...
class IoU_Cal:
    ...
    @classmethod
    def NWD(cls, pred, target, self=None, sigma=1.0):
        self = self if self else cls(pred, target)
        distance = self.l2_center
        nwd = torch.exp(-distance / (4 * sigma ** 2))
        return self._scaled_loss(1 - nwd)

    @classmethod
    def WiseIoU_v3(cls, pred, target, self=None, alpha=1.5):
        self = self if self else cls(pred, target)
        base_loss = (1 - self.iou) ** alpha
        return self._scaled_loss(base_loss)
